param(
    ...
    [string[]]$Include = @("AGENTS.md", "docker-compose.yaml", ".dockerignore", ".gitignore", ".agents", ".codex", "services"),
    [switch]$DryRun
)
...
$skipPatterns = @(
    ...
    '(^|[\\/])config[\\/]cloudflared[\\/](data|logs)([\\/]|$)',
    '(^|[\\/])config[\\/]whisper[\\/](data|logs|models)([\\/]|$)',
    '(^|[\\/])services[\\/][^\\/]+[\\/]data([\\/]|$)',
    '(^|[\\/])services[\\/][^\\/]+[\\/]logs([\\/]|$)',
    '(^|[\\/])__pycache__([\\/]|$)',
    '(^|[\\/])\.pytest_cache([\\/]|$)',
    ...
    foreach ($pathInfo in $resolved) {
        ...
        if ((Get-Item -LiteralPath $abs).PSIsContainer) {
            Get-ChildItem -LiteralPath $abs -File -Recurse -Force | ForEach-Object {
                ...
                if (-not (Test-Skipped $rel)) { $files.Add($_) }
            }
        } else {
            $rel = Get-RelativePathCompat $root $abs
            if (-not (Test-Skipped $rel)) { $files.Add((Get-Item -LiteralPath $abs)) }
        }
    }
}
...
if (-not (Test-Path -LiteralPath $zipPath)) {
    throw "No se creo el ZIP de backup: $zipPath"
}
