...
$remote = @'
set -e
cd /volume1/docker/buscador-rd
sudo env BUILDX_GIT_INFO=false docker compose up -d --build btdigg-rd
echo "--- docker ps ---"
sudo docker ps --filter 'name=^/btdigg-rd$' --format '{{.Names}} {{.Status}} {{.Ports}}'
echo "--- http ---"
err_file="$(mktemp)"
ok=0
last_code=""
for attempt in $(seq 1 20); do
  last_code="$(curl -sS -o /dev/null -w '%{http_code}' http://127.0.0.1:9007/ 2>"$err_file" || true)"
